...
public:
  ...
  void draw(int data[ANIM_DATA_SLOTS])
  {
    ...
    if (mirrorMode == 1 || mirrorMode == 4)
      segmentSize /= 2;
    else if (mirrorMode == 2 || mirrorMode == 5)
      segmentSize /= 3;
    else if (mirrorMode == 3 || mirrorMode == 6)
      segmentSize /= 4;
    if (segmentSize < 1)
      segmentSize = 1;
    ...
    if (pixMode == 23)
      ...
      colorMode = COLOR_QUAD;
    else if (btw(pixMode, 6, 11))
    ...
    else if (colorMode == COLOR_TRI || colorMode == COLOR_QUAD)
    {
      ...
      for (int i = 0; i < segmentSize; i++)
      {
        ...
        if (dashPart < 0 || dashPart > dashSplit)
          dashPart = 0; // use 0 if outside of the dash (=> background)
    ...
    if (colorMode == COLOR_BI || colorMode == COLOR_PICKER)
    {
      ...
      {
        dashLength = max(2, scale255(segmentSize, data[6])); // pix_start
        dashOffset = scale255(segmentSize, data[7]);         // pix_pos
      }
      ...
      if (pixMode == 2 || pixMode == 8)
      {
        ...
        for (int i = 0; i < segmentSize; i++)
          if ((i + dashOffset) / dashLength % 2 == 1)
            segment[i] = backColor;
      }
    ...
    if (btw(strobeMode, 3, 10) || btw(strobeMode, 12, 19) || btw(strobeMode, 20, 25))
    {
      ...
      if (btw(strobeMode, 3, 10))
        strobeSeuil = (data[8] - 31) * 1000 / 69; // 0->1000    strobeMode >= 3 && strobeMode <= 10
      else if (btw(strobeMode, 12, 19))
        strobeSeuil = (data[8] - 121) * 1000 / 79; // 0->1000    strobeMode >= 12 && strobeMode <= 19
      else if (btw(strobeMode, 20, 25))
        strobeSeuil = (data[8] - 201) * 1000 / 54; // 0->1000    strobeMode >= 20

      // apply random Seuil
      for (int i = 0; i < segmentSize; i++)
        if (random(1000) > strobeSeuil)
          segment[i] = {CRGBW::Black};
    }
