...
void ezHeader::_recalculate()
{
    ...
    { // start from left, set x values
        _widgets[n].x = x;
        if (_widgets[n].leftover)
        { // until "leftover" widget (usually "title")
    ...
    if (we_have_leftover)
    { // Then start from right setting x values
        x = TFT_W;
        for (int8_t n = _widgets.size() - 1; n >= 0; n--)
        {
            if (_widgets[n].leftover)
            { // and set width of leftover widget to remainder
...
String ezButtons::poll()
{
    ...
    if (!_key_release_wait)
    {
        ...
        else if (_btn_bc != "" && m5.BtnB.isPressed() && m5.BtnC.isPressed())
        {
            keystr = ez.leftOf(_btn_bc, "|", true);
            _key_release_wait = true;
        }
        else if (_btn_ac != "" && m5.BtnA.isPressed() && m5.BtnC.isPressed())
        {
        ...
        // LONG PRESS
        else if (_btn_a_l != "" && m5.BtnA.wasReleasefor(ez.theme->longpress_time))
        {
            keystr = ez.leftOf(_btn_a_l, "|", true);
            m5.BtnA.read();
        }
        else if (_btn_b_l != "" && m5.BtnB.wasReleasefor(ez.theme->longpress_time))
        {
            keystr = ez.leftOf(_btn_b_l, "|", true);
            m5.BtnB.read();
        }
        else if (_btn_c_l != "" && m5.BtnC.wasReleasefor(ez.theme->longpress_time))
        {
        ...
        // SHORT PRESS
        else if (_btn_a_s != "" && m5.BtnA.wasReleasefor(ez.theme->shortpress_time))
        {
            keystr = ez.leftOf(_btn_a_s, "|", true);
            m5.BtnA.read();
        }
        else if (_btn_b_s != "" && m5.BtnB.wasReleasefor(ez.theme->shortpress_time))
        {
            keystr = ez.leftOf(_btn_b_s, "|", true);
            m5.BtnB.read();
        }
        else if (_btn_c_s != "" && m5.BtnC.wasReleasefor(ez.theme->shortpress_time))
        {
...
#endif
    if (keystr != "") Serial.println(keystr);
    return keystr;
}
    ...
    while (true)
    {
        ...
        case 1:
        {
            ...
            while (true)
            {
                ...
                if (b == "right" && _brightness <= 240)
                    _brightness += 16;
                if (!_brightness)
                    _brightness = 255;
                if (b == "left" && _brightness > 16)
                    _brightness -= 16;
                if (_brightness == 239)
                    _brightness = 240;
...
{
    if (state)
        M5.lcd.fillRect(_text_cursor_x, _text_cursor_y, _text_cursor_w, _text_cursor_h, ez.theme->input_fgcolor);
    if (!state)
        M5.lcd.fillRect(_text_cursor_x, _text_cursor_y, _text_cursor_w, _text_cursor_h, ez.theme->input_bgcolor);
    ...
    while (true)
    {
        if (redraw)
        {
            if (!readonly && cursor_x && cursor_y)
                m5.lcd.fillRect(cursor_x, cursor_y, cursor_width, cursor_height, ez.screen.background()); //Remove current cursor
            cursor_x = cursor_y = 0;
            tmp_buttons = buttons;
            ...
            if (lines.size() > 0)
            {
                for (int8_t n = offset; n < offset + lines_per_screen; n++)
                {
                    ...
                    if (!readonly && sol != -1 && cursor_pos >= sol && cursor_pos <= eol && n < lines.size())
                    { // if cursor is on current line
        ...
        if (key == (String) char(191))
        { // left arrow on FACES keyboard
            if (cursor_pos > 0)
            {
                cursor_pos--;
                redraw = true;
        ...
        if (key == (String) char(193))
        { // right arrow on FACES keyboard
        ...
        if (key == (String) char(183) || key == (String) char(192))
        { // up or down arrow on FACES keyboard
...
{
    if (str == "back" || str == "exit" || str == "done" || str == "Back" || str == "Exit" || str == "Done")
        return true;
    return false;
}
...
ezMenu::~ezMenu()
{
    if (this == M5ez::_currentMenu)
        M5ez::_currentMenu = nullptr;
}
